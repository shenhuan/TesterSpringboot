...
import java.util.*;
//下面2个注解是用来在做测试类时，使用autowired注解生成bean不为null的
...
public class test1 {
    ...
    @Test
    public void setName(){
        redisUtil.set("shh","shenhuanhuan");
        System.out.println("shh===>"+redisUtil.get("shh"));
    }

    private  void lottery(){
        Integer allAmount= 200;//总抽奖次数
        //参与抽奖的人数及金额
        ...
        Map<Integer, Offset> userOffsetMap = new HashMap<Integer, Offset>();
        float percent;//比例
        int span;//跨度
        int start = 0;//区间开始
        int end; //区间结束
        for(User user : users){
            //比例是单个人的投入金额/所有人加起来的总金额，如果有小数则4舍五入一下
            percent = getPercent(user.getAmount(), allAmount);
            //跨度就是：根据比例，在100之间计算出区间跨度，进一步来计算中奖区间
     ...
     * @return 跨度
     */
    private int getRandom(float percent) {
     ...
     * @return 比例
     */
    private float getPercent(int amount, Integer allAmount) {
        // 比例计算出来会有很多小数，为了看得清晰，要四舍五入一下
        return new BigDecimal(amount/ allAmount.floatValue()).setScale(2,BigDecimal.ROUND_HALF_UP).floatValue();
    }
